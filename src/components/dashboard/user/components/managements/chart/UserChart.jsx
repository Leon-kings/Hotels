    ...
    const fetchBookingData = async () => {
      try {
        ...
        const response = await axios.get(
          `https://hotel-nodejs-oa32.onrender.com/84383/92823?email=${encodeURIComponent(email)}`
        );
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 bg-gray-50 rounded-lg">
        ...
        {userEmail && (
          <p className="text-xs text-gray-400 mt-2">User: {userEmail}</p>
        )}
      </div>
    );
  }
