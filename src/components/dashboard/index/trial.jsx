...
const BookingSearch = ({ onSearchResults }) => {
  ...
  const resetSearch = () => {
    ...
    onSearchResults(null); // Passing null to indicate reset
  };
...
export default function AdminBookingView() {
  ...
  const handleUpdate = async (bookingId) => {
    ...
    // 2. Prepare the payload (with date formatting if needed)
    const payload = {
      ...formData,
      // Ensure dates are in correct format (backend may expect ISO string or YYYY-MM-DD)
    ...
    try {
      ...
      if (!updatedBooking) {
        throw new Error("Server response did not include updated booking data");
      }
      ...
      // 6. Reset editing state and show success
      setEditingBooking(null);
      alert("Booking updated successfully!");
      ...
      // 9. Optional: Revert optimistic updates if implemented
      // setAllBookings(originalBookings);
    } finally {
      // 10. Any cleanup (e.g., hide loading spinner)
      // setIsLoading(false);
    }
  };
...
export const MessagesManager = () => {
  ...
  const handleUpdate = async (id) => {
    try {
      ...
      // 4. Update state
      setMessages(
        messages.map((msg) => (msg._id === id ? { ...msg, ...editForm } : msg))
      );
      setEditingId(null);
