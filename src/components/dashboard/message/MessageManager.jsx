...
export const MessagesManager = () => {
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      // Initialize isRead property if not present
      const messagesWithReadStatus = (response.data.data || response.data || []).map(msg => ({
        ...msg,
  ...
  const toggleReadStatus = async (id) => {
    try {
      ...
      await axios.patch(
        `https://hotel-nodejs-oa32.onrender.com/63729/892308/${id}`,
        { isRead: !messages.find(msg => msg._id === id).isRead }
      );
    } catch (err) {
              ...
              currentMessages.map((message) => (
                ...
                >
                  <td className="px-2 py-4 whitespace-nowrap text-center">
                    {message.isRead ? (
                      <MarkEmailRead className="text-green-500" titleAccess="Read" />
                    ) : (
                      <MarkEmailUnread className="text-red-500" titleAccess="Unread" />
                    )}
                  </td>
