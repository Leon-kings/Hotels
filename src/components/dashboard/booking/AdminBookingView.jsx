...
const BookingSearch = ({ onSearchResults }) => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      switch(searchCriteria.searchType) {
        ...
        case 'date':
          if (searchCriteria.dateRange.startDate) {
            queryParams.startDate = new Date(searchCriteria.dateRange.startDate).toISOString();
          }
          if (searchCriteria.dateRange.endDate) {
            queryParams.endDate = new Date(searchCriteria.dateRange.endDate).toISOString();
          }
          break;
        case 'status':
      ...
      const response = await axios.get('https://hotel-nodejs-oa32.onrender.com/84383/92823', {
        params: queryParams
      });
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mb-6">
      ...
      <form onSubmit={handleSearch}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          {['name', 'bookingId', 'email'].includes(searchCriteria.searchType) && (
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700">
                {searchTypes.find(t => t.value === searchCriteria.searchType)?.label}
              </label>
              <input
                type="text"
                name="searchTerm"
          ...
          {searchCriteria.searchType === 'date' && (
            <div className="col-span-1 md:col-span-2 space-y-2">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex items-center justify-center text-gray-500 sm:py-0 py-2">to</div>
                <div className="flex-1">
                  <input
                    type="date"
                    name="endDate"
        ...
        <div className="flex items-center justify-end gap-4">
          ...
          <button
            ...
            className={`inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${isSearching ? 'opacity-75 cursor-not-allowed' : ''}`}
          >
            {isSearching ? (
              <>
...
export default function AdminBookingView() {
  ...
  const handleDelete = async (id) => {
    try {
      if (window.confirm('Are you sure you want to delete this booking?')) {
        await axios.delete(`https://hotel-nodejs-oa32.onrender.com/84383/92823/${id}`);
  ...
  return (
    <div className="container mx-auto p-4">
      <BookingSearch onSearchResults={(results) => {
        if (results === null) {
            ...
            <div className="flex space-x-2">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  ...
                  className={`px-3 py-1 rounded ${currentPage === page ? 'bg-blue-600 text-white' : 'bg-blue-500 text-white hover:bg-blue-600'}`}
                >
                  {page}
                </button>
              ))}
